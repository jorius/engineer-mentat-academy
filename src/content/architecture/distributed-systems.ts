...
export const questions: Question[] = [
  {
    ...
    hint: 'Cover what microservices really solve (team and deploy independence), what they cost a small team, the anti-pattern to avoid, and concrete triggers for splitting.',
  },
  {
...
export function solution(messages, maxAttempts) {
  ...
  return { processed, deadLettered };
    ...
    hint: 'Check the retry bound with `maxAttempts = 3` by counting attempts, and classify each outcome as transient or permanent before deciding to retry.',
  },
  {
...
export function solution(events: IncomingEvent[]): number[] {
  ...
  },
  {
    ...
    hint: 'Cover who generates the key, what the server stores with it, how a retry is answered, how concurrent duplicates stay race-safe, and how long keys live.',
  },
  {
