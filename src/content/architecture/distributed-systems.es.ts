...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'distributed-systems-kafka-ordering-keys': {
    prompt:
      'Los eventos `created`, `paid` y `shipped` de una misma orden deben procesarse en orden. El topic tiene 12 partitions y el productor envía los mensajes **sin key**. A veces los consumidores ven `shipped` antes que `paid`. ¿Cuál es la solución correcta?',
    options: {
    ...
    explanation:
      'Kafka solo garantiza el orden **dentro de una partition**. Sin key, los mensajes se reparten entre partitions y se consumen en paralelo, así que se pierde el orden por orden de compra. Usar como key la entidad cuyo orden importa (`orderId`) envía por hash todos sus eventos a una sola partition, mientras que las distintas órdenes se siguen repartiendo entre las 12, lo que mantiene el paralelismo. Una sola partition también funciona, pero limita el throughput a un solo consumidor. Los timestamps de distintos productores no son un orden confiable. Dos detalles más: mantén habilitado el productor idempotente para que los reintentos no reordenen ni dupliquen dentro de una partition, y recuerda que agregar partitions más adelante cambia el mapeo de key a partition.',
  },
  ...
  'distributed-systems-idempotency-keys-api': {
    ...
    modelAnswer:
      'El cliente genera una key única (un UUID) por operación lógica y la envía en cada reintento de esa operación. El servidor guarda la key, acotada a quien llama (tenant o usuario), junto con un hash del cuerpo de la solicitud y un estado, en una tabla con una restricción de unicidad. En la primera solicitud inserta la key como `in-progress` dentro del mismo límite transaccional que el trabajo del pago, realiza el cobro y guarda el código de estado y el cuerpo de la respuesta finales asociados a la key. Un reintento con la misma key y el mismo cuerpo devuelve la respuesta guardada sin volver a cobrar; la misma key con un cuerpo distinto es un bug del cliente y recibe un 422; un reintento que llega mientras el original sigue en curso recibe un 409 (o espera), y la restricción de unicidad hace que esto sea seguro ante carreras. Las keys expiran después de una ventana más larga que cualquier política de reintentos del cliente (por ejemplo 24 horas). Aguas abajo, paso la misma key al proveedor de pagos, ya que Stripe y APIs similares también soportan idempotency keys, así la garantía se mantiene de punta a punta.',
    rubric: [
