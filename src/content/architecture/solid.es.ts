...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'solid-ocp-exporter-registry': {
    ...
    explanation:
      'OCP significa que extiendes el comportamiento **agregando** código, no editando código que ya funciona. Un registro (un mapa de estrategias) permite que cada formato viva en su propio módulo, y los tres puntos de llamada no vuelven a cambiar. La opción A funciona, pero hay que repetirla en cada switch duplicado, que es justamente el olor de shotgun surgery al que apunta OCP. La opción C sigue editando un switch, solo que en una subclase, y D agrega un flag booleano que no escala más allá de dos formatos.\n\nLa salvedad pragmática: un único `switch` sobre un conjunto de casos estable y cerrado (por ejemplo los tres estados de un semáforo) está bien. OCP rinde cuando el conjunto crece y la ramificación está duplicada.',
  },
