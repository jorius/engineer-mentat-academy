...
export const questions: Question[] = [
  {
    ...
    hint: 'Recall what `select` and `include` each control in a Prisma result, and the rules for combining them in one query.',
  },
  {
    ...
    hint: 'Recall what each Prisma migration command is designed for, and what a production pipeline needs from the one it runs.',
  },
  {
    ...
    hint: 'Recall how the classic `query` relation load strategy differs from the newer `join` strategy.',
  },
  {
...
export function solution(table: Post[], authorIds: number[], chunkSize: number) {
  ...
  return { queries, titles: authorIds.map((id) => byAuthor.get(id) ?? []) };
    ...
    hint: "Handle duplicate ids, keep every call within the `chunkSize` limit, and remember the result must follow the caller's order, not the database's.",
  },
  {
