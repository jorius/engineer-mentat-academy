...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'react-testing-library-user-event': {
    ...
    explanation: '`fireEvent.change` despacha un único evento sintético `change` con el valor ya asignado, saltándose `keydown`, `keypress`, `input` y `keyup`, así que el handler que bloquea las letras nunca se ejecuta. `user.type` simula lo que hace el navegador con cada carácter (foco, eventos de teclado, eventos de input, respetando `preventDefault`). En user-event v14 cada API devuelve una promesa; olvidar el `await` (c) significa que la aserción se ejecuta antes de que terminen los eventos. Crea el `user` con `userEvent.setup()` antes de renderizar.',
  },
