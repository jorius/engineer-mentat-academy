...
export const questions: Question[] = [
  {
    ...
    hint: "Recall the priority order RTL's docs give for queries, and the principle behind that order.",
  },
  {
  ...
  },
  {
    ...
    hint: 'List the browser events a real keystroke fires, then check which approach dispatches them the way a user would, and how its API must be called.',
  },
  {
...
The test fails with "Unable to find an element with the text: Ada Lovelace". What is the correct fix?`,
    ...
    hint: 'The name appears only after the mocked request resolves; recall which RTL tools are meant for UI that shows up asynchronously.',
  },
  {
    ...
    hint: 'Recall how `waitFor` runs its callback and when it resolves, then judge each snippet by what it relies on.',
  },
  {
