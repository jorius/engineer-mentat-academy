...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'redux-toolkit-immer-reassign': {
    ...
    explanation: '`createSlice` ejecuta los case reducers a través de Immer: `state` es un proxy draft, e Immer registra las **mutaciones** de ese draft (el `push` de `added`, la asignación de `couponApplied`) o acepta un valor de reemplazo **devuelto** (como en `cleared`). `state = ...` solo vuelve a enlazar una variable local; el draft no se toca y no se devuelve nada, así que Immer devuelve el state original. Escribe `return initialState` en su lugar. La otra trampa de Immer: puedes mutar el draft **o** devolver un valor nuevo, no las dos cosas; hacer ambas lanza un error.',
  },
