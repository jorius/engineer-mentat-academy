...
export const translations: Record<string, QuestionTranslation> = {
  'redux-reducer-rules': {
    ...
    options: {
      ...
      e: '```js\nreturn state;\n```\n(para un tipo de acción que el reducer no maneja)',
    },
    explanation: 'Un reducer debe ser una función pura de `(state, action)`: sin mutaciones (`state.count++`), sin efectos secundarios como llamadas de red (`fetch`) y sin valores no deterministas como ids aleatorios o `Date.now()`, porque reproducir las mismas acciones (time-travel debugging, tests, reproducir un registro de acciones grabado) debe producir el mismo state. Genera los ids en el action creator (el callback `prepare` de Redux Toolkit) y pon los efectos secundarios en thunks, listeners o middleware. Devolver el state existente para acciones desconocidas es obligatorio: mantiene la referencia sin cambios, así los suscriptores saben que nada cambió.',
    hint: 'Pregúntate en cada caso: ¿reproducir las mismas acciones daría siempre el mismo estado, sin mutar el anterior ni tocar el mundo exterior?',
  },
  'redux-reducer-immutability-fix': {
    ...
    hint: 'react-redux detecta los cambios por referencia, así que copia cada nivel que cambies con spread, `map` o `filter` en vez de mutarlo.',
  },
  'redux-middleware-order': {
    ...
    hint: 'Piensa en el middleware como capas de una cebolla: ¿cuál es la capa exterior y cuándo se notifica a los suscriptores respecto al código después de `next(action)`?',
  },
  ...
  'redux-toolkit-immer-reassign': {
    ...
    explanation: '`createSlice` ejecuta los case reducers a través de Immer: `state` es un proxy draft, e Immer registra las **mutaciones** de ese draft (el `push` de `added`, la asignación de `couponApplied`) o acepta un valor de reemplazo **devuelto** (como en `cleared`). `state = ...` solo vuelve a enlazar una variable local; el draft no se toca y no se devuelve nada, así que Immer devuelve el state original. Escribe `return { items: [], coupon: null };` en su lugar (o saca ese objeto a una constante `initialState` con nombre y devuélvela). La otra trampa de Immer: puedes mutar el draft **o** devolver un valor nuevo, no las dos cosas; hacer ambas lanza un error.',
    hint: 'Recuerda cómo funciona el draft de Immer dentro de un case reducer de `createSlice`, y cómo se entera Immer de cuál debe ser el siguiente estado.',
  },
  'redux-async-thunk-vs-rtk-query': {
    ...
    hint: 'Separa la caché del servidor del estado del cliente y cubre cómo la invalidación por tags corrige las pantallas desactualizadas.',
  },
};
