...
export const translations: Record<string, QuestionTranslation> = {
  'react-lifecycle-mount-unmount-effect': {
    ...
    explanation: 'Un efecto con `[]` se ejecuta después del primer commit, y la función que devuelve es el cleanup que React llama al desmontar. Sin el arreglo, el efecto y su cleanup se ejecutan después de **cada** render, así que te vuelves a suscribir en cada render. Llamar a `unsubscribe()` justo después de `subscribe()` cancela la suscripción de inmediato. La variante con `useMemo` pone un efecto secundario en `useMemo`, que se ejecuta durante el render y puede ejecutarse más de una vez o descartarse.\n\nEl mejor modelo mental no es "ciclo de vida" sino "sincronizar con un sistema externo": si la suscripción dependiera de una prop como `storeId`, iría en el arreglo de dependencias, y ese único efecto también reemplazaría a `componentDidUpdate`.',
    hint: 'Piensa en para qué sirve la función que devuelve un efecto y en cómo el arreglo de dependencias controla cada cuánto se ejecutan el setup y el cleanup.',
  },
  ...
  'react-hooks-rules-violations': {
    ...
    explanation: 'React identifica cada hook por su **orden de llamada** dentro de un render, así que cada render debe llamar a los mismos hooks en el mismo orden, y solo desde componentes o custom hooks. Un return temprano antes de un hook vuelve condicional al hook. Un bucle cambia la cantidad cuando `fields` cambia. Un hook en un helper de un event handler se ejecuta completamente fuera del render. Los custom hooks como `useThemeColor()` son solo funciones cuyo nombre empieza con `use` y que llaman hooks en su propio nivel superior, que es la forma aprobada de compartir lógica de hooks.\n\n(El `use()` de React 19 es la única excepción que puede llamarse de forma condicional.)',
    hint: 'React distingue los hooks por su orden de llamada. Revisa si cada caso llama los mismos hooks en el mismo orden en cada render, desde un componente o un custom hook.',
  },
  'react-usestate-batched-increments': {
    ...
    hint: 'Fíjate en el valor que tiene `count` dentro del handler de este render y compara pasar un valor con pasar una función updater.',
  },
  'react-usereducer-cart-reducer': {
    ...
    explanation: 'Un reducer se ejecuta durante el render, así que debe ser puro: mismo `(state, action)` de entrada, mismo state de salida, sin mutaciones. Un `add` tentador parece inmutable, pero no lo es:\n\n```ts\nconst existing = state.lines.find((l) => l.id === action.id);\nexisting.qty++;\nreturn { ...state };\n```\n\nMuta el objeto de línea al que todavía apunta el state anterior; por eso el test registra todo el historial.\n\nPor qué `useReducer` en lugar de varias llamadas a `useState`: el *qué pasó* (acciones desde los event handlers) se separa del *cómo cambia el state* (el reducer), el reducer se puede probar con tests unitarios sin ningún componente, y una sola acción como `clear` puede describir una interacción del usuario aunque cambie muchos campos. Devolver la misma referencia para acciones desconocidas le permite a React evitar el re-render.',
    hint: 'Crea objetos y arreglos nuevos en cada cambio (`map`, `filter`, spread), nunca modifiques una línea a la que el estado anterior todavía apunta y conserva la misma referencia para acciones desconocidas.',
  },
  'react-fetch-effect-race': {
    ...
    hint: 'Cubre por qué las respuestas pueden llegar desordenadas, qué puede hacer el cleanup del efecto con el request anterior (`AbortController` o un flag para ignorarlo) y qué librería usarías.',
  },
  ...
  'react-exhaustive-deps-infinite-loop': {
    ...
    hint: 'Explica cómo se comparan las dependencias cuando son funciones y objetos creados durante el render, y luego las formas de eliminar la dependencia en vez de silenciar el linter.',
  },
  'react-debounced-value-logic': {
    ...
    hint: 'Recorre la línea de tiempo una vez comparando el momento de disparo de cada cambio (`t + delay`) con el `t` del siguiente, y recuerda que al último cambio no lo cancela nada.',
  },
  'react-rerender-triggers': {
    prompt: '¿Cuáles de estos hacen que `Child` (sin envolver en `React.memo`, en una app que no usa el React Compiler) vuelva a renderizar? Selecciona todas las que apliquen.',
    options: {
  ...
  'react-derived-state-anti-pattern': {
    ...
    modelAnswer: '`filtered` es state derivado: siempre se puede calcular a partir de `tickets` y `query`, así que guardarlo crea una segunda fuente de verdad que hay que mantener sincronizada. El efecto se ejecuta solo después de que el render hace commit, así que cada cambio primero hace commit de un render con la lista `filtered` obsoleta y luego vuelve a renderizar (cuando `tickets` cambia después de un fetch, el usuario incluso puede ver ese frame obsoleto). También es frágil: si olvidas una dependencia, la lista se queda obsoleta sin avisar, y `useState(tickets)` solo usa `tickets` como valor inicial. Reemplaza el state y el efecto con un valor calculado durante el render: `const filtered = tickets.filter(...)`, envuelto en `useMemo(..., [tickets, query])` solo si el filtro es mediblemente costoso. La regla es state mínimo (datos crudos más la intención del usuario) y derivar todo lo demás. El mismo razonamiento aplica a "reiniciar el state cuando cambia una prop": usa una `key` en lugar de un efecto. Copiar una prop al state solo es correcto cuando el usuario edita un borrador a propósito, y en ese caso la prop debería llamarse `initialX`.',
    rubric: [
    ...
    hint: 'Pregúntate si `filtered` es información nueva o algo que puedes calcular a partir de props y estado durante el render, y cuánto cuesta el efecto en renders extra.',
  },
  'react-state-management-choice': {
    ...
    hint: 'Clasifica cada pieza según quién es su dueño (el servidor, la URL, un componente, toda la app) y cada cuánto cambia, y luego elige dónde vive cada una.',
  },
  ...
  'react-large-form-performance': {
    ...
    hint: 'Parte de cuánto del árbol se vuelve a renderizar por tecla, y luego cubre cómo reducir ese alcance (inputs no controlados o campos aislados) y sacar la validación del camino de cada tecla.',
  },
  'react-memoization-trio': {
    ...
    hint: 'Explica cómo dependen los tres entre sí a través de la igualdad de referencias y cuánto cuesta memoizar cuando no se ha medido ningún render costoso.',
  },
  'react-transition-vs-deferred': {
    prompt: 'En una app de React 19 renderizada con `createRoot`, un componente padre que no es tuyo pasa `query` (actualizado en cada tecla) a tu `<BigList query={query} />`, que filtra y renderiza 20,000 filas. Escribir en la caja de búsqueda tiene lag. Solo puedes cambiar `BigList`. ¿Cuál es la herramienta correcta?',
    options: {
      a: '`const deferred = useDeferredValue(query)`, y construir las filas filtradas a partir de `deferred` dentro de `useMemo` (o pasar `deferred` a una lista envuelta en `memo`)',
