...
export const translations: Record<string, QuestionTranslation> = {
  'dynamodb-valid-key-condition': {
    ...
    explanation:
      'Un `Query` debe apuntar exactamente a una partición con **igualdad** sobre la partition key, porque la partition key se pasa por un hash para encontrar la partición de almacenamiento; no hay un orden sobre el que hacer una búsqueda por prefijo o por rango (b). La sort key se guarda ordenada dentro de una partición, así que admite `=`, `<`, `<=`, `>`, `>=`, `BETWEEN` y `begins_with`. `contains` (d) no es una condición de clave; solo se permite en una `FilterExpression`, que se ejecuta después de leer los items. Consultar solo por sort key (c) requiere un GSI que tenga ese atributo como partition key, o un `Scan`.',
  },
  'dynamodb-gsi-vs-lsi': {
    ...
    explanation:
      'Un LSI es "local" porque vive en la misma partición que los items base: misma partition key, otra sort key, solo se crea junto con la tabla y agrega el límite de 10 GB por item collection. Un GSI tiene su propia partition key y sort key, se puede agregar o quitar en cualquier momento, tiene su propia capacidad y se replica de forma asíncrona, así que sus lecturas son **solo eventualmente consistentes** (b es falsa). Las claves de un GSI no son únicas (e es falsa): muchos items pueden compartir la misma partition key y sort key del GSI, y DynamoDB no tiene restricciones de unicidad más allá de la primary key (la unicidad se emula con un put condicional sobre un item aparte dentro de una transacción). Un GSI con capacidad insuficiente también puede provocar throttling en las escrituras a la tabla base.',
  },
  ...
  'dynamodb-hot-partition-throttling': {
    ...
    rubric: [
      ...
      'Explica que el modo on-demand o la adaptive capacity no elimina el límite por clave',
