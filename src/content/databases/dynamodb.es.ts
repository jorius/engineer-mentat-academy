...
export const translations: Record<string, QuestionTranslation> = {
  'dynamodb-valid-key-condition': {
    ...
    hint: 'Recuerda qué operadores acepta la partition key frente a la sort key, y qué funciones solo caben en un `FilterExpression`.',
  },
  'dynamodb-gsi-vs-lsi': {
    ...
    hint: 'Piensa en dónde vive cada índice respecto a la partición base, cuándo se puede crear y cómo se replica.',
  },
  'dynamodb-filter-after-limit': {
    ...
    hint: 'Recuerda el orden de las operaciones en un `Query`: cuándo se aplica `Limit` respecto al `FilterExpression`.',
  },
  'dynamodb-order-keys-builder': {
    ...
    hint: 'Arma los strings con template literals; usa `String.prototype.padStart` para el id y `toUpperCase` para el estado.',
  },
  'dynamodb-hot-partition-throttling': {
    ...
    hint: 'Explica el límite de throughput por partición, por qué una fecha como clave pone un día entero en una sola clave y cuánto cuesta el write sharding en las lecturas.',
  },
  'dynamodb-single-table-design-tradeoffs': {
    ...
    hint: 'Cubre la ubicación de varios tipos de entidad bajo una misma partition key, las claves y GSI sobrecargados, y cuándo perjudica tener que conocer los patrones de acceso de antemano.',
  },
};
