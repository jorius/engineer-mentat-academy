...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'testing-unit-test-doubles': {
    ...
    options: {
      ...
      b: 'Una función mock (vocabulario de Vitest y Jest; Meszaros lo llama test spy): devuelve datos predefinidos **y** registra las llamadas, y el test verifica la interacción (a quién se llamó y con qué)',
      c: 'Un fake: una implementación ligera que funciona, como un servidor de correo en memoria',
      d: 'Un wrapper de `vi.spyOn` sobre el cliente de correo real: el correo real se sigue enviando',
    },
    explanation:
  ...
  'testing-backend-testcontainers': {
    prompt:
      'Los tests de tu capa de repositorio hacen mock del driver de la base de datos o corren contra SQLite en memoria, y todos pasan. En producción, una consulta falla porque una migración renombró `customer_id` a `client_id`, y otro bug se cuela porque SQLite y Postgres (usado en producción) ordenan `NULL` de forma distinta. ¿Cuál es el test más barato que habría detectado **ambos**?',
    options: {
  ...
  'testing-frontend-implementation-details': {
    ...
    options: {
      ...
      e: 'Si el formulario fuera un componente de clase, leer el estado de su instancia con Enzyme: `wrapper.state(\'isOpen\')` es igual a `true`',
    },
    explanation:
