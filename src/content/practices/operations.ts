...
export function solution(logs: LogLine[], correlationId: string): { path: string[]; firstError: string | null } {
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'b', text: 'Require every paging alert to be actionable and linked to a runbook; review the last month of pages and delete or downgrade the alerts that, on review, needed no action' },
  ...
  },
  {
    ...
    language: 'typescript',
    prompt:
      'Implement `solution(samples)` that takes request latencies in milliseconds (unsorted) and returns an object with the keys `p50`, `p95` and `p99`, using the **nearest-rank** method: sort ascending, then the p-th percentile is the value at 1-based rank `ceil(p / 100 * n)`. For an empty array return `null` for all three. Do not mutate the input.',
