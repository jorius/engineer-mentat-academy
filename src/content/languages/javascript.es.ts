...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'javascript-shallow-copy-nested': {
    prompt:
      '¿Cuáles de estas opciones producen una copia **profunda** de `{ a: { b: [1, 2] }, d: new Date() }` que conserve el `Date`? Selecciona todas las que correspondan.',
    explanation:
      'Spread y `Object.assign` copian un solo nivel. Pasar por `JSON` convierte un `Date` en string y descarta funciones y `undefined`. `structuredClone` maneja objetos anidados, Dates, Maps y Sets, pero no funciones ni prototipos de clase.',
  },
  ...
  'javascript-hoisting-tdz-core': {
    prompt:
      '```js\nconsole.log(a);\nconsole.log(typeof f);\nlet a = 1;\nfunction f() {}\n```\n¿Qué pasa en la primera línea?',
    explanation:
      '`let` tiene hoisting, pero queda sin inicializar hasta que se ejecuta su declaración, así que leerla lanza un error. Con `var` se imprimiría `undefined`; la declaración de función tiene hoisting completo, pero aquí nunca se llega a ella.',
    options: {
  ...
  'javascript-null-vs-undefined-core': {
    prompt:
      'En las entrevistas preguntan la diferencia entre `null` y `undefined`. Demuestra que sabes cómo se comporta cada uno en código real: ¿qué imprime esto, un valor por línea?',
    explanation:
  ...
  'javascript-promise-chain-recovery-core': {
    prompt:
      'En las entrevistas te piden explicar las promesas. Sigue la resolución, la propagación de errores y `finally` a lo largo de este código. ¿Qué imprime, un valor por línea?',
    explanation:
  ...
  'javascript-immutability-freeze-core': {
    prompt:
      'En las entrevistas preguntan sobre inmutabilidad. El fragmento se ejecuta como módulo ES (modo estricto). ¿Qué imprime, un valor por línea?',
    explanation:
  ...
  'javascript-arrow-vs-regular-core': {
    prompt:
      'En las entrevistas preguntan la diferencia entre las funciones flecha y las funciones regulares. ¿Qué imprime esto, un valor por línea?',
    explanation:
  ...
  'javascript-destructuring-defaults-core': {
    prompt: 'En las entrevistas te piden explicar la desestructuración. ¿Qué imprime esto, un valor por línea?',
    explanation:
  ...
  'javascript-spread-shallow-core': {
    prompt: 'En las entrevistas preguntan para qué sirve el operador spread. ¿Qué imprime esto, un valor por línea?',
    explanation:
  ...
  'javascript-set-semantics-core': {
    prompt: 'En las entrevistas preguntan para qué sirve el objeto `Set`. ¿Qué imprime esto, un valor por línea?',
    explanation:
  ...
  'javascript-coercion-to-primitive-core': {
    prompt:
      'Continuación de `==` vs `===`: ¿cómo se convierten los objetos y `null`? ¿Qué imprime esto, un valor por línea?',
    explanation:
  ...
  'javascript-equality-explain': {
    ...
    explanation:
      'La respuesta común se queda en "`==` aplica coerción, `===` no". Los entrevistadores presionan por el **algoritmo** (ToNumber, ToPrimitive, el caso especial de `null`/`undefined`), por **`NaN` y `Object.is`**, y por una **política** que realmente harías cumplir. Ejercicios relacionados: `javascript-equality-coercion-core` y `javascript-coercion-to-primitive-core`.\n\n**Dilo en voz alta:** "`===` compara tipo y valor sin conversión; `==` ejecuta un algoritmo de coerción con suficientes casos borde como para que lo prohíba por lint salvo en `x == null`, y recurro a `Object.is` cuando importan `NaN` o el cero con signo."',
    modelAnswer:
  ...
  'javascript-null-undefined-explain': {
    ...
    explanation:
      'La respuesta común es la definición. Una respuesta senior agrega las **diferencias observables** que causan bugs (los valores por defecto ignoran `null`, JSON descarta `undefined`, `typeof null`) y una **convención** que sobreviva a los límites de serialización. Ejercicio relacionado: `javascript-null-vs-undefined-core`.\n\n**Dilo en voz alta:** "`undefined` significa que nadie lo asignó, `null` significa que alguien lo dejó vacío a propósito; los valores por defecto y JSON los tratan distinto, así que uso `??` para manejar ambos y mantengo una convención clara en el límite de la API, como ausente frente a `null` en un PATCH."',
    modelAnswer:
  ...
  'javascript-event-loop-explain': {
    ...
    explanation:
      'La respuesta de referencia común describe una sola "cola de callbacks". Ese es el modelo junior; el modelo senior tiene **dos colas con reglas de vaciado distintas** más **oportunidades de renderizado**, y predice la salida real. Ejercicios relacionados: `javascript-event-loop-async-await-core` y `javascript-event-loop-order-basic`.\n\n**Dilo en voz alta:** "Después de cada tarea, el motor vacía toda la cola de microtareas antes de renderizar o tomar la siguiente tarea, así que los callbacks de promesas se adelantan a los temporizadores, y tanto una tarea larga como una cadena interminable de microtareas congelan la página."',
    modelAnswer:
  ...
  'javascript-var-let-const-explain': {
    ...
    explanation:
      'La respuesta común cubre el scope y la redeclaración. Los entrevistadores profundizan en la **TDZ**, en **`const` frente a la inmutabilidad** y en el comportamiento del **binding en los loops**, porque ahí viven los bugs. Ejercicios relacionados: `javascript-var-let-const-loop-core` y `javascript-hoisting-tdz-core`.\n\n**Dilo en voz alta:** "`var` tiene scope de función y vale `undefined` en silencio antes de su línea; `let` y `const` tienen scope de bloque y lanzan un error en la zona muerta temporal. Uso `const` por defecto, sabiendo que congela el binding, no el objeto."',
    modelAnswer:
  ...
  'javascript-prototypal-inheritance-explain': {
    ...
    explanation:
      'La respuesta de referencia común describe la cadena. La capa senior es la **asimetría entre lectura y escritura** (la búsqueda recorre la cadena, la asignación oculta), lo que hace **`new`** paso a paso y que **las clases son el mismo mecanismo**. Ejercicios relacionados: `javascript-prototype-shared-state-core` y `javascript-static-vs-instance-core`.\n\n**Dilo en voz alta:** "Las lecturas recorren la cadena de prototipos y las escrituras caen en el propio objeto; las clases son solo una forma más agradable de armar la misma cadena, y por eso el estado mutable en un prototipo se filtra a todas las instancias."',
    modelAnswer:
  ...
  'javascript-hoisting-explain': {
    ...
    explanation:
      'La respuesta de referencia común dice que las declaraciones "se mueven arriba". Los entrevistadores que miden seniority quieren la explicación de la **fase de creación** y la **TDZ**, incluido su efecto de ocultamiento y los distintos errores que producen las llamadas anticipadas. Ejercicios relacionados: `javascript-hoisting-tdz-core` y `javascript-var-let-const-loop-core`.\n\n**Dilo en voz alta:** "El hoisting es el motor creando los bindings de un scope antes de ejecutarlo: las declaraciones de función quedan listas para llamarse, `var` empieza como `undefined`, y `let`, `const` y `class` quedan en la zona muerta temporal y lanzan un error hasta que se ejecuta su línea."',
    modelAnswer:
  ...
  'javascript-method-vs-function-explain': {
    ...
    explanation:
      'La respuesta común ("un método es una función asignada a una propiedad de un objeto") es correcta pero superficial. La versión senior es que la diferencia vive en **la llamada**, no en la función, más la semántica concreta de las **definiciones de método** y el trade-off entre prototipo y campo en las clases. Ejercicio relacionado: `javascript-method-vs-function-core`.\n\n**Dilo en voz alta:** "Un método es solo una función llamada a través de un objeto, así que `this` es ese objeto; sácala del objeto y vuelve a ser una función simple, por eso elijo a conciencia entre métodos de prototipo y campos de clase con funciones flecha."',
    modelAnswer:
  ...
  'javascript-promises-explain': {
    ...
    explanation:
      'La respuesta común cubre los tres estados. Una respuesta senior agrega la **semántica del encadenamiento** (lo que devuelve el handler decide la siguiente promesa), el **momento de las microtareas**, la **elección del combinador** según su comportamiento ante fallos y los errores que detectas en un review. Ejercicios relacionados: `javascript-promise-chain-recovery-core` y `javascript-promise-combinators-choice`.\n\n**Dilo en voz alta:** "Una promesa se resuelve una sola vez, y cada `then` devuelve una promesa nueva definida por lo que su handler devuelve o lanza; esa única regla explica el encadenamiento, la propagación de errores y la recuperación."',
    modelAnswer:
  ...
  'javascript-sync-vs-async-explain': {
    ...
    explanation:
      'La respuesta común explica bloqueante frente a no bloqueante. La distinción senior es la **concurrencia sin paralelismo** y la decisión entre **trabajo limitado por I/O y trabajo limitado por CPU**, incluido el mito de que marcar una función como `async` evita que bloquee. Ejercicio relacionado: `javascript-promise-executor-sync-core`.\n\n**Dilo en voz alta:** "En JavaScript, async es concurrencia, no paralelismo: libera el hilo mientras el host espera la I/O, pero el trabajo de CPU sigue bloqueando a menos que lo divida o lo mueva a un worker."',
    modelAnswer:
  ...
  'javascript-event-delegation-explain': {
    ...
    explanation:
      'La respuesta común explica la delegación y el bubbling. Los detalles senior son **`closest` más la comprobación de contención** para targets anidados, los **eventos que no hacen bubbling** y cómo **`stopPropagation`** rompe en silencio a los delegados. Ejercicio relacionado: `javascript-event-delegation-closest-core`.\n\n**Dilo en voz alta:** "Pongo un solo listener en el contenedor y resuelvo el target real con `event.target.closest(selector)`, que maneja el markup anidado y los elementos agregados después; para el foco escucho `focusin`, porque `focus` no hace bubbling."',
    modelAnswer:
  ...
  'javascript-array-map-explain': {
    ...
    explanation:
      'La respuesta común dice que `map` es puro. Siendo precisos: `map` no muta el array, pero solo es tan puro como su callback, y el resultado es un array nuevo **superficial**. De un senior se espera que conozca las trampas de la firma del callback y que detecte el mal uso en un review. Ejercicio relacionado: `javascript-map-parseint-core`.\n\n**Dilo en voz alta:** "`map` es para transformaciones uno a uno que devuelven un array nuevo; si ignoro el resultado, lo que quiero es `forEach`, y si el callback es async necesito `Promise.all` alrededor."',
    modelAnswer:
  ...
  'javascript-functional-programming-explain': {
    ...
    explanation:
      'La respuesta común enumera funciones de primera clase, de orden superior y puras. Un senior muestra **dónde** la aplica (un núcleo puro, reducers, componentes de React) y **dónde se detiene** (I/O en los bordes, legibilidad por encima de la astucia). Ejercicios relacionados: `javascript-pipe-functional-core` y `javascript-memoize-cache-key-core`.\n\n**Dilo en voz alta:** "Mantengo la lógica de negocio como funciones puras sobre datos inmutables y empujo los efectos secundarios hacia los bordes; eso hace que el núcleo sea trivial de probar, y es exactamente el modelo que ya usan los reducers y los componentes de React."',
    modelAnswer:
  ...
  'javascript-destructuring-explain': {
    ...
    explanation:
      'La respuesta común muestra la sintaxis básica. Los entrevistadores revisan la **semántica de los bordes**: los valores por defecto y `null`, desestructurar `undefined`, la ambigüedad de las llaves entre bloque y objeto, y que nada se copia en profundidad. Ejercicio relacionado: `javascript-destructuring-defaults-core`.\n\n**Dilo en voz alta:** "Los valores por defecto de la desestructuración solo se activan con `undefined`, y desestructurar `undefined` en sí lanza un error, así que para los parámetros de opciones escribo `function f({ timeout = 1000 } = {})`."',
    modelAnswer:
  ...
  'javascript-spread-explain': {
    ...
    explanation:
      'La respuesta común define el spread. Una respuesta senior trata sobre la **semántica de la copia**: superficial, solo propiedades enumerables propias, prototipo perdido. De ahí vienen los bugs de producción, sobre todo en las actualizaciones de estado. Ejercicio relacionado: `javascript-spread-shallow-core`.\n\n**Dilo en voz alta:** "El spread hace una copia superficial de las propiedades enumerables propias, así que es perfecto para actualizaciones inmutables del nivel superior, pero los objetos anidados siguen compartidos; para una copia profunda de verdad uso `structuredClone`."',
    modelAnswer:
  ...
  'javascript-static-vs-instance-explain': {
    ...
    explanation:
      'La respuesta común dice dónde se llama cada método. Los agregados senior son **dónde vive cada uno** (prototipo frente a constructor), la **herencia de estáticos** con `this` apuntando a la subclase y el **criterio de diseño** entre factories y estado global oculto. Ejercicio relacionado: `javascript-static-vs-instance-core`.\n\n**Dilo en voz alta:** "Los métodos de instancia viven en el prototipo y trabajan con el estado de un objeto; los métodos estáticos viven en la clase, lo que los hace adecuados para factories como `User.fromJson` e inadecuados para cualquier cosa que guarde en silencio estado mutable compartido."',
    modelAnswer:
  ...
  'javascript-expression-vs-statement-explain': {
    ...
    explanation:
      'La respuesta común da definiciones. De un senior se espera que conecte la distinción con el **parseo**: los mismos caracteres significan cosas distintas en posición de sentencia y de expresión, lo que explica el bug de la función flecha con literal de objeto, la ASI después de `return` y la regla de `{}` en JSX. Ejercicio relacionado: `javascript-arrow-object-literal-core`.\n\n**Dilo en voz alta:** "Las expresiones producen valores y las sentencias hacen cosas; las llaves de JSX y los template literals solo aceptan expresiones, y una `{` inicial se interpreta como bloque en posición de sentencia, por eso una función flecha que devuelve un objeto necesita paréntesis."',
    modelAnswer:
  ...
  'javascript-strict-mode-explain': {
    ...
    explanation:
      'La respuesta común es la definición. La respuesta senior nombra **cambios de comportamiento específicos** y sabe que **los módulos y las clases son estrictos por defecto**, por eso la mayor parte del código moderno es estricto sin la directiva. Ejercicios relacionados: `javascript-strict-mode-core` y `javascript-immutability-freeze-core`.\n\n**Dilo en voz alta:** "El modo estricto convierte fallos silenciosos en errores, como las escrituras en objetos congelados y las globales accidentales, y hace que `this` sea `undefined` en las llamadas simples; los módulos ES y las clases siempre son estrictos, así que el código moderno lo obtiene gratis."',
    modelAnswer:
