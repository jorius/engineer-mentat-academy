...
export const translations: Record<string, QuestionTranslation> = {
  'javascript-event-loop-order-basic': {
    ...
    hint: 'Separa el trabajo en código síncrono, callbacks de promesas y callbacks de temporizadores, y recuerda qué cola se vacía por completo antes de la siguiente tarea.',
  },
  ...
  'javascript-closure-counter-independence': {
    ...
    hint: 'Pregúntate si cada llamada a `outer` crea su propia copia de `n` o si ambas funciones devueltas comparten una.',
  },
  ...
  'javascript-array-methods-some-every': {
    ...
    hint: 'Usa `Array.prototype.every` y `Array.prototype.some`, y piensa qué devuelve cada uno con un array vacío.',
  },
  'javascript-promise-combinators-choice': {
    ...
    hint: 'Compara los combinadores según cómo reaccionan cuando una entrada se rechaza, y luego piensa cómo acotar cada llamada para que un servicio lento no fije la latencia de la página.',
    modelAnswer:
  ...
  'javascript-equality-coercion-core': {
    ...
    hint: 'Recuerda cómo la igualdad laxa convierte un string vacío y trata a `null`, qué hace `+` con dos objetos y si `NaN` es igual a algo.',
  },
  ...
  'javascript-null-vs-undefined-core': {
    ...
    hint: 'Separa el valor que reporta el motor cuando falta algo del valor que un programa asigna a propósito, y recuerda cómo tratan a cada uno `typeof`, `JSON.stringify` y los parámetros por defecto.',
  },
  'javascript-arrow-object-literal-core': {
    ...
    hint: 'En cada fragmento, pregúntate si las llaves o los paréntesis están en posición de sentencia o de expresión; el cuerpo de una flecha que abre con `{` se interpreta como un bloque.',
  },
  'javascript-event-loop-async-await-core': {
    ...
    hint: 'El código antes del primer `await` se ejecuta de forma síncrona; cada continuación tras un `await` y cada reacción de promesa es un trabajo encolado tras el script actual, y todos se vacían antes de cualquier callback de temporizador.',
  },
  'javascript-promise-executor-sync-core': {
    ...
    hint: 'La función que pasas a `new Promise` se ejecuta de inmediato, y llamar a `resolve` no la detiene; solo los callbacks registrados se difieren.',
  },
  ...
  'javascript-var-let-const-loop-core': {
    ...
    hint: 'Cuenta cuántas variables del bucle existen con `var` frente a `let`, revisa si `var` se escapa del bucle y recuerda que `const` impide reasignar, no mutar.',
  },
  'javascript-prototype-shared-state-core': {
    ...
    hint: 'Pregúntate dónde vive realmente el array: las lecturas recorren la cadena de prototipos, `push` muta el objeto que encuentra ahí y una asignación normal crea una propiedad propia.',
  },
  'javascript-static-vs-instance-core': {
    ...
    hint: 'Ubica cada método: los estáticos viven en el constructor, los de instancia en su prototipo, y dentro de un estático `this` es la clase sobre la que se llamó.',
  },
  'javascript-this-call-site-core': {
    ...
    hint: 'Aplica las reglas de `this` en orden de prioridad a cada llamada: `new`, `bind`, `call`/`apply`, llamada de método y luego llamada simple en modo estricto; las flechas las ignoran todas.',
  },
  ...
  'javascript-promise-chain-recovery-core': {
    ...
    hint: 'Una promesa se resuelve una sola vez e ignora las llamadas posteriores; cada handler devuelve una promesa nueva, un `throw` salta al `catch` más cercano y un handler de limpieza deja pasar el valor sin cambios.',
  },
  ...
  'javascript-map-parseint-core': {
    ...
    hint: 'Enumera los tres argumentos que `map` pasa a su callback y qué hace `parseInt` con un segundo argumento; piensa también en cómo trata `map` los huecos de un array disperso.',
  },
  ...
  'javascript-immutability-freeze-core': {
    ...
    hint: '`Object.freeze` es superficial y solo lanza errores al escribir en modo estricto; revisa también qué métodos de array cambian el array en su lugar y cuáles devuelven una copia.',
  },
  'javascript-arrow-vs-regular-core': {
    ...
    hint: 'Las flechas no tienen `this`, `arguments` ni `[[Construct]]` propios, y `call` o `bind` no pueden cambiar su receptor.',
  },
  'javascript-destructuring-defaults-core': {
    ...
    hint: 'Los valores por defecto solo se aplican a un tipo de valor ausente, no a `null`; pregúntate también qué pasa cuando el valor que desestructuras no existe.',
  },
  'javascript-spread-shallow-core': {
    ...
    hint: 'Spread copia un solo nivel: los arrays anidados siguen compartidos, reasignar una propiedad solo afecta la copia, las claves posteriores sobrescriben a las anteriores y un string se expande en sus caracteres.',
  },
  'javascript-set-semantics-core': {
    ...
    hint: 'La unicidad de `Set` usa SameValueZero: sin coerción de tipos, `NaN` es igual a sí mismo y los objetos se comparan por referencia; la iteración sigue el orden de inserción.',
  },
  'javascript-strict-mode-core': {
    ...
    hint: 'El modo estricto convierte los fallos silenciosos en errores y cambia el receptor de una llamada simple a función; recuerda que los módulos ES son estrictos automáticamente.',
  },
  'javascript-event-delegation-closest-core': {
    ...
    hint: 'Piensa en qué elemento es `event.target` cuando el clic cae sobre algo anidado dentro del botón.',
    options: {
      a:
  ...
  'javascript-data-binding-core': {
    ...
    hint: 'Cubre cada dirección por separado (eventos del DOM hacia el modelo, observación con setters, `Proxy` o suscripciones de vuelta a la vista), el riesgo de ciclos de retroalimentación y por qué React eligió un flujo en un solo sentido.',
    modelAnswer:
  ...
  'javascript-coercion-to-primitive-core': {
    ...
    hint: 'Sigue qué hint de ToPrimitive usa cada operador (`valueOf` o `toString` primero), el caso especial de `null` en la igualdad laxa y el paso ToNumber que aplican los operadores relacionales.',
  },
  'javascript-async-operations-explain': {
    ...
    hint: 'Empieza por quién espera realmente (el host, no el hilo de JS), presenta callbacks, promesas y async/await como una sola evolución, y luego cubre paralelismo, cancelación y manejo de errores.',
    modelAnswer:
  ...
  'javascript-equality-explain': {
    ...
    hint: 'Ve más allá de la frase corta: los pasos de conversión de la igualdad laxa, el caso especial de `null`/`undefined`, `NaN` y `Object.is`, y una política de review que aplicarías.',
    modelAnswer:
  ...
  'javascript-closures-explain': {
    ...
    hint: 'Destaca que los closures capturan variables vivas y no valores, y conéctalo con un uso real (estado privado, factories), el bucle con `var`, los closures obsoletos de React y el tiempo de vida en memoria.',
    modelAnswer:
  ...
  'javascript-null-undefined-explain': {
    ...
    hint: 'Nombra las diferencias observables que causan bugs (valores por defecto, serialización JSON, `typeof`) y propone una convención de equipo que sobreviva a los límites de una API.',
    modelAnswer:
  ...
  'javascript-event-loop-explain': {
    ...
    hint: 'Describe dos colas con reglas de vaciado distintas más las oportunidades de renderizado, y muestra que el modelo predice el orden real y puede bloquear el renderizado.',
    modelAnswer:
  ...
  'javascript-var-let-const-explain': {
    ...
    hint: 'Cubre alcance y redeclaración, y luego las preguntas de seguimiento: la zona muerta temporal, `const` frente a la inmutabilidad real, las variables por iteración en los bucles y tu opción por defecto.',
    modelAnswer:
  ...
  'javascript-prototypal-inheritance-explain': {
    ...
    hint: 'Explica la asimetría entre lectura y escritura en la cadena, qué hace `new` paso a paso y cómo la sintaxis `class` se apoya en el mismo mecanismo.',
    modelAnswer:
  ...
  'javascript-this-explain': {
    ...
    hint: 'Enuncia las reglas de enlace en orden de precedencia, corrige el mito del objeto global para el código estricto, los módulos y las clases, y nombra el bug del receptor perdido con sus soluciones.',
    modelAnswer:
  ...
  'javascript-hoisting-explain': {
    ...
    hint: 'Reemplaza la historia de "se mueve arriba" por la fase de creación del alcance, y cubre la zona muerta temporal, su efecto de sombreado y los distintos errores al usar algo antes de tiempo.',
    modelAnswer:
  ...
  'javascript-method-vs-function-explain': {
    ...
    hint: 'Argumenta que la diferencia está en la expresión de llamada, y luego cubre qué cambia la sintaxis de definición de métodos y el trade-off entre método de prototipo y campo flecha en las clases.',
    modelAnswer:
  ...
  'javascript-promises-explain': {
    ...
    hint: 'Ve más allá de los estados: qué hace el valor de retorno de un handler con la siguiente promesa, el momento de las microtareas, elegir combinadores por su comportamiento ante fallos y los errores que detectas en review.',
    modelAnswer:
  ...
  'javascript-sync-vs-async-explain': {
    ...
    hint: 'Distingue concurrencia de paralelismo en un solo hilo, clasifica el trabajo en limitado por I/O o por CPU y aborda el mito de que `async` hace que el código no bloquee.',
    modelAnswer:
  ...
  'javascript-event-delegation-explain': {
    ...
    hint: 'Cubre cómo encuentras el objetivo real dentro de marcado anidado y confirmas que pertenece al contenedor, qué eventos no burbujean y cómo un `stopPropagation` en otro lado te rompe.',
    modelAnswer:
  ...
  'javascript-array-map-explain': {
    ...
    hint: 'Sé preciso sobre la pureza y la copia superficial, recuerda los tres argumentos del callback y las trampas que causan, y enumera los malos usos que señalarías en un review.',
    modelAnswer:
  ...
  'javascript-functional-programming-explain': {
    ...
    hint: 'Muestra dónde lo aplicas en el día a día (un núcleo puro, reducers, componentes) y dónde te detienes (I/O en los bordes, legibilidad antes que ingenio).',
    modelAnswer:
  ...
  'javascript-arrow-vs-regular-explain': {
    ...
    hint: 'Convierte la lista de diferencias en una regla de decisión para callbacks frente a métodos y constructores, y nombra las trampas que aparecen en código real.',
    modelAnswer:
  ...
  'javascript-destructuring-explain': {
    ...
    hint: 'Enfócate en la semántica de los bordes: cuándo se aplican los valores por defecto, desestructurar un valor ausente, la ambigüedad de llaves entre bloque y objeto, y que nada se copia en profundidad.',
    modelAnswer:
  ...
  'javascript-spread-explain': {
    ...
    hint: 'Centra la respuesta en la semántica de la copia: qué tan profunda es, qué propiedades se copian, qué pasa con el prototipo y dónde eso te afecta en actualizaciones de estado.',
    modelAnswer:
  ...
  'javascript-static-vs-instance-explain': {
    ...
    hint: 'Cubre dónde vive cada tipo (prototipo frente a constructor), cómo se heredan los estáticos y qué significa `this` dentro de ellos, y cuándo una factory estática es mejor que un estado global oculto.',
    modelAnswer:
  ...
  'javascript-expression-vs-statement-explain': {
    ...
    hint: 'Conecta la distinción con el parsing: los mismos caracteres significan cosas distintas según la posición, lo que explica el bug de la flecha con objeto literal, el ASI tras `return` y las llaves de JSX.',
    modelAnswer:
  ...
  'javascript-immutability-explain': {
    ...
    hint: 'Relaciona la inmutabilidad con la igualdad por referencia y la detección de cambios en React y Redux, y conoce los límites de cada herramienta, desde `Object.freeze` hasta `readonly`.',
    modelAnswer:
  ...
  'javascript-strict-mode-explain': {
    ...
    hint: 'Nombra cambios de comportamiento concretos en lugar de la definición, y explica qué código ya es estricto sin la directiva.',
    modelAnswer:
  ...
  'javascript-set-explain': {
    ...
    hint: 'Explica cómo se decide la unicidad, por qué la comprobación de pertenencia supera a un array y la trampa de serialización cuando un `Set` cruza el límite de una API.',
    modelAnswer:
