...
export const questions: Question[] = [
  ...
  },
  {
    ...
    source: 'topic-list',
    explanation:
      "The single-argument `groupingBy(classifier)` collects each group into a `List`; in OpenJDK today that means a `HashMap<K, ArrayList<V>>` (`a` is true, describing that current OpenJDK behavior), but the `Collectors.groupingBy` Javadoc promises none of it — not the concrete map type, not the values' `List` implementation or mutability, not iteration order, only that elements are grouped by key. The two-argument overload, `groupingBy(classifier, downstream)`, applies the given downstream `Collector` to each group instead of defaulting to `toList()`; `averagingDouble(Employee::salary)` reduces each group to the mean of its `salary` values, so `\"eng\"` (90 000 and 110 000) averages to `100000.0` (`b` is true). `groupingBy`'s contract makes no ordering promise at all — OpenJDK's current default happens to be an unordered `HashMap`, not a `LinkedHashMap`, so `c` is false and department order is not something you can rely on. Streams are non-mutating by design: collecting never writes back into the source list or its elements, it only builds a new result, so `d` is false — nothing about `employees` changes. To control the resulting `Map` implementation (for stable iteration, sorting, concurrency, etc.) you need the three-argument overload `groupingBy(classifier, mapFactory, downstream)`, supplying something like `TreeMap::new` as the `mapFactory` (`e` is true).",
  },
  {
