...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'typescript-basics-types-erased': {
    prompt: 'Esto compila sin errores con `strict`. ¿Qué imprime en tiempo de ejecución, una línea por cada `console.log`?',
    explanation:
    ...
    hint: 'Los tipos y las aserciones `as` desaparecen en el JavaScript emitido, así que pregúntate cuáles son realmente los valores en tiempo de ejecución y qué comprueba de verdad el cuerpo del type guard.',
  },
  ...
  'typescript-interface-vs-type-open': {
    ...
    hint: 'Elige una opción por defecto y luego nombra las diferencias mecánicas: la fusión de declaraciones, cómo se reportan los conflictos al extender y lo que solo pueden expresar los alias de tipo.',
    modelAnswer:
  ...
  'typescript-declaration-merging': {
    ...
    hint: 'Recuerda qué hace TypeScript con dos interfaces del mismo nombre en el mismo alcance, y en qué se diferencia de dos alias de tipo.',
    options: {
      a: 'Falta la propiedad `height`: las dos declaraciones se fusionaron, así que `Box` exige tanto `width` como `height`.',
  ...
  'typescript-enum-runtime-output': {
    prompt:
      'Los enums son una de las pocas características de TypeScript que emiten código en tiempo de ejecución. ¿Qué imprime esto, una línea por cada `console.log`?',
    explanation:
    ...
    hint: 'Los enums numéricos emiten un objeto con mapeo inverso y se autoincrementan desde el miembro anterior; los enums de strings no tienen entradas inversas.',
  },
  'typescript-as-const-satisfies': {
    prompt: 'Este es el patrón común de "enum sin `enum`". ¿Qué imprime, una línea por cada `console.log`?',
    explanation:
    ...
    hint: '`as const` y `readonly` existen solo en tiempo de compilación; pregúntate qué hace el JavaScript emitido cuando el código muta el array o busca en él.',
  },
  ...
  'typescript-discriminated-union-render': {
    ...
    hint: 'Haz `switch` sobre `state.status` para que cada `case` se reduzca a un solo miembro, y pasa el valor a `assertNever` en el `default`.',
  },
  ...
  'typescript-partial-spread-undefined': {
    prompt:
      'Una pantalla de configuración combina un patch `Partial` sobre los valores por defecto. Esto compila con `strict`. ¿Qué imprime, una línea por cada `console.log`?',
    explanation:
    ...
    hint: 'El spread de objetos copia las propiedades propias aunque su valor esté vacío, y sin `exactOptionalPropertyTypes` una clave de `Partial` acepta un `undefined` explícito.',
  },
  ...
  'typescript-boundary-validation-open': {
    prompt:
      'Un servicio consume JSON de un webhook de un tercero y de una llamada a una herramienta de un LLM. Hoy el código hace esto:\n```ts\nconst event = (await res.json()) as OrderEvent;\n```\n¿Qué tiene de malo eso y cómo tipas correctamente estos límites?',
    explanation:
    ...
    hint: 'Parte de que los tipos no garantizan nada sobre los datos externos, y luego cubre el parseo con un schema en tiempo de ejecución en el borde, los tipos inferidos, el camino de fallo y los límites que no son HTTP.',
    modelAnswer:
      'El cast `as` es una aserción sin comprobar: los tipos se borran, así que los datos mal formados fluyen hasta lo profundo del sistema y fallan lejos de su causa o, peor, se persisten. Un type predicate escrito a mano (`value is OrderEvent`) no es más seguro: el compilador confía en lo que devuelva su cuerpo. Todo lo que cruza un límite de confianza (bodies HTTP, webhooks, mensajes de colas, variables de entorno, `localStorage`, la salida de un LLM o de una herramienta) debería entrar como `unknown` y **parsearse** en el borde con un schema de runtime como zod, valibot o ajv. Derivo el tipo estático a partir del schema (`type OrderEvent = z.infer<typeof OrderEvent>`) para que el validador y el tipo no puedan desincronizarse. Parsea, no valides: el parser devuelve un valor tipado o un error estructurado, y más allá del borde el código de dominio nunca vuelve a ver `unknown`. El camino de fallo es explícito: un 400 para las APIs síncronas, una dead-letter queue para webhooks y mensajes, un reintento acotado o un prompt de reparación para la salida del LLM, y un log con el correlation id en todos los casos. También decido por límite si las claves desconocidas se eliminan o se rechazan, y versiono el schema cuando cambia el productor. La configuración de entorno recibe el mismo tratamiento al arrancar, para que el proceso falle rápido en lugar de en la primera petición.',
    rubric: [
