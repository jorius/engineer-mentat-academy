...
export const questions: Question[] = [
  ...
  },
  {
    ...
    source: 'topic-list',
    explanation:
      'An `async Task` method returns a `Task` the caller can `await`; any exception thrown inside is captured on that `Task` and rethrown at the `await`, so an ordinary `try`/`catch` around the call sees it. `async void` is fire-and-forget: there is no `Task` to await, and an exception thrown inside it is raised on the current `SynchronizationContext` instead of being catchable by the caller, which typically crashes the process. It exists mainly for UI event handlers, which cannot return a value. A plain `void` method is not `async` at all, so it cannot `await`; blocking on `.GetAwaiter().GetResult()` inside it just makes the call synchronous and does not give the caller anything to `await` — and if this runs on a thread with a captured single-threaded `SynchronizationContext` (a UI thread, or classic ASP.NET), blocking like that risks a deadlock, because the awaited continuation needs that same thread to resume. `Task<void>` is not valid C# — `void` cannot be used as a type argument, so option `d` does not compile; use `Task` for "no return value" and `Task<T>` when you need one.',
  },
  {
