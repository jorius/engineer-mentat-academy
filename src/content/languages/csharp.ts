...
export const questions: Question[] = [
  {
    ...
    hint: "Ask whether a `struct` is a value type or a reference type, and what assignment does with each kind.",
  },
  {
    ...
    hint: "Think about what the caller needs back in order to `await` the call, and where an exception goes when there is nothing to await.",
  },
  {
  ...
  },
  {
    ...
    hint: "Recall how `await` unwraps a faulted task's `AggregateException`, and in which order `WhenAll` records the inner exceptions.",
  },
  {
    ...
    hint: "Compare the contract of `First` with its `OrDefault` sibling when the sequence has nothing in it, and what the default of `int` is.",
  },
  {
    ...
    hint: "Think about which comparer the synthesized `Equals` uses per field, whether `List<T>` overrides equality, and what `EqualityContract` checks.",
  },
  {
    ...
    hint: "Cover what capturing the `SynchronizationContext` means for callers that block (WPF vs ASP.NET Core), and the single-consumption contract of `ValueTask<T>`.",
  },
  {
    ...
    hint: "Separate what runs when `Squares` is called from what runs each time the result is enumerated; think deferred execution and materialization.",
  },
];
