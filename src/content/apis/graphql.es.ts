...
export const translations: Record<string, QuestionTranslation> = {
  'graphql-over-under-fetching': {
    prompt:
      'Una pantalla móvil muestra el nombre de un usuario y los títulos de sus últimas 3 órdenes. Con la API REST llama a `GET /users/7` (devuelve 40 campos) y luego a `GET /users/7/orders` (devuelve las órdenes completas con sus líneas). ¿Qué problemas resuelve una query GraphQL como `{ user(id: 7) { name orders(last: 3) { title } } }`?',
    options: {
  ...
  'graphql-dataloader-per-request-context': {
    ...
    explanation:
      '`context` es el objeto por petición que comparten todos los resolvers de una operación: ahí viven el usuario autenticado, los handles de la base de datos y los loaders. Crear los loaders ahí acota tanto la **ventana de batch** como el **caché de memoización** a una sola petición.\n\n- **a** es el distractor peligroso: un caché a nivel de módulo nunca se invalida (datos obsoletos después de las escrituras), crece sin límite y puede servirle a un usuario datos cargados con los permisos de otro.\n- **c** crea un loader nuevo por cada llamada a un campo, así que no hay nada con qué agrupar y el N+1 vuelve.\n- **d** `info` es el AST de la query y metadatos del schema, no un lugar para estado por petición.\n\nPara un caché entre peticiones, pon Redis (o caché HTTP) *debajo* del loader, con TTLs e invalidación explícitos.',
  },
  ...
  'graphql-operational-costs': {
    ...
    explanation:
      '**a**: los GET de REST se pueden cachear por URL en todas las capas (navegador, CDN, reverse proxy). Un único endpoint POST anula eso; las persisted queries (un hash en lugar de la query completa) recuperan el caché por GET.\n\n**b**: el éxito parcial es normal en GraphQL (`data` más `errors`), así que monitoreas el array `errors` y las métricas a nivel de resolver, no solo los códigos HTTP.\n\n**c**: "peticiones por minuto" significa poco cuando una sola query puede expandirse a millones de filas. Agregas límites de profundidad de query, análisis de costo, topes de paginación y, para APIs públicas, allowlists de persisted queries.\n\n**d**: ver DataLoader.\n\n**e** es falsa: las APIs GraphQL normalmente evolucionan **sin** versiones. Agregas campos libremente y marcas los viejos como obsoletos con `@deprecated`, y luego los eliminas cuando la telemetría de uso por campo muestra que ningún cliente los usa.',
  },
