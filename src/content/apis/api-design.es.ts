...
export const translations: Record<string, QuestionTranslation> = {
  'api-design-offset-drift-predict': {
    ...
    hint: 'El offset ubica las filas por posición y el cursor por valor; sigue dónde cae cada uno después del insert al inicio del feed.',
  },
  'api-design-offset-vs-cursor-tradeoffs': {
    ...
    explanation:
      '- **Sin saltos de página con cursores**: verdadera, y es la razón principal por la que las tablas de administración con números de página siguen usando offsets.\n- **Un `OFFSET` profundo es más lento**: verdadera; el costo crece linealmente con el offset, mientras que la paginación por keyset va directo al punto del índice.\n- **Clave de orden única con desempate**: verdadera; si dos filas comparten un `createdAt` en el límite de una página, `createdAt < :last` se salta la segunda. Ordena por `(createdAt, id)` y compara la tupla.\n- **Conteo total exacto gratis**: falsa; un total necesita un `COUNT(*)` aparte, que es costoso en tablas grandes. Muchas APIs devuelven `hasMore`/`nextCursor` en lugar de un total.\n- **Cursores opacos**: verdadera; un cursor opaco es un contrato ("devuélveme lo que te di"), no un formato que los clientes puedan construir o parsear.\n\nRegla general: offset para listas de administración pequeñas y estables con números de página; cursor para feeds, scroll infinito, APIs de sincronización y tablas grandes.',
    hint: 'Piensa en qué hace la base de datos con un `OFFSET` profundo, qué necesita un cursor de la clave de orden y qué puede y no puede decirte un cursor sobre la posición y los totales.',
  },
  'api-design-keyset-cursor-page': {
    ...
    hint: 'Compara la tupla completa `(createdAt, id)` con el cursor decodificado, no solo el timestamp; decodifícalo con `atob` y `JSON.parse`.',
  },
  'api-design-breaking-changes': {
    ...
    explanation:
      'Un cambio es breaking cuando un cliente que funcionaba ayer falla hoy sin haber cambiado su código.\n\n- Los cambios **aditivos** son seguros: el campo opcional `giftMessage` en la respuesta y el nuevo endpoint `/refunds`, *siempre que* los clientes sigan la regla del tolerant reader e ignoren los campos desconocidos.\n- **Eliminar o renombrar** cualquier cosa que un cliente lea (`total` a `totalAmount`), **endurecer** las reglas de entrada (hacer obligatorio `currency`) y **cambiar tipos** (`id` de número a string, que rompe a los clientes tipados y las comparaciones con `===`) son breaking.\n\nLa zona gris: agregar un valor a un enum de la respuesta puede romper a los clientes que hacen un switch exhaustivo sobre él. Documenta los enums como abiertos ("espera valores nuevos") desde el primer día.',
    hint: 'Aplica una prueba a cada cambio: ¿un cliente que funcionaba ayer fallaría hoy sin cambiar su código, suponiendo que ignora los campos desconocidos?',
  },
  'api-design-versioning-and-deprecation': {
    ...
    hint: 'Cubre cómo evitar rupturas con cambios aditivos, dónde vive la versión, una sola implementación detrás de adaptadores y el retiro con headers `Deprecation`/`Sunset` y datos de uso.',
  },
  'api-design-error-envelope': {
    ...
    options: {
      ...
      c: '`422 Unprocessable Content` con `Content-Type: application/problem+json` y este body:\n\n```json\n{\n  "type": "https://api.acme.io/errors/validation",\n  "title": "Invalid request",\n  "status": 422,\n  "errors": [\n    { "field": "email", "code": "invalid_format" },\n    { "field": "age", "code": "min", "min": 0 }\n  ]\n}\n```',
  ...
  'api-design-202-accepted-meaning': {
    ...
    hint: 'Recuerda qué tanto promete un 202 comparado con un 201, y a qué tipo de recurso apunta el header `Location`.',
  },
  'api-design-long-running-jobs': {
    ...
    hint: 'Acepta rápido y expón primero el job como un recurso; luego agrega streaming de progreso, webhooks para los partners y claves de idempotencia para los reintentos.',
  },
};
