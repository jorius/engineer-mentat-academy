...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'api-security-distributed-rate-limiting': {
    ...
    rubric: [
      ...
      'Pone límites en capas: borde (WAF, usage plans de API Gateway) más nivel de aplicación',
  ...
  'api-security-public-endpoint-controls': {
    ...
    explanation:
      '- **a** limita la fuerza bruta, el spam y los DoS; agrega un CAPTCHA o proof-of-work si el abuso persiste.\n- **b** es la regla central: **nunca confíes en el input**. Valida en la frontera (zod, class-validator, JSON Schema) con allowlists y límites de tamaño; esto también bloquea la asignación masiva de campos que no pretendías aceptar.\n- **e** se ocupa del input que *sí* aceptaste: las queries parametrizadas evitan la inyección SQL, y la codificación de salida (más una CSP) evita el XSS almacenado cuando un administrador ve el mensaje.\n\n**c** es el malentendido clásico: **CORS lo aplican los navegadores**, y solo controla si una página de otro origen puede *leer* la respuesta. `curl`, los scripts y los bots lo ignoran por completo. **d**: los atacantes no usan tu formulario. La validación del lado del cliente es UX, no seguridad.',
  },
