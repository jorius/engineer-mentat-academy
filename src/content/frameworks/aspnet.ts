...
export const questions: Question[] = [
  {
    ...
    options: [
      { id: 'a', text: 'Middleware runs only once, at application startup; the code inside `app.Use` executes when the host-starting `app.Run()` at the bottom of `Program.cs` is called, not for each incoming HTTP request' },
      { id: 'b', text: 'Middleware is a chain of components, each given the `HttpContext` and a delegate to the next component; each can run code both before and after calling that delegate, so requests flow "in" through the chain in registration order and responses flow back "out" in reverse order' },
  ...
  },
  {
    ...
    options: [
      { id: 'a', text: '`IOrderService orders` must be resolved manually inside the constructor with `HttpContext.RequestServices.GetService<IOrderService>()`; constructor injection is not supported for ASP.NET Core controllers' },
      { id: 'b', text: '`[ApiController]` is what makes the route reachable at all; without it, the `[HttpGet]` and `[Route]` attributes are ignored and `GetById` can never be invoked' },
      { id: 'c', text: "Because `GetById` returns `IActionResult`, the response is always serialized as XML unless the client explicitly sends an `Accept: application/json` header — JSON is only a negotiated fallback format" },
      { id: 'd', text: '`IOrderService orders` is supplied by dependency injection when the controller is activated for a request, provided `IOrderService` was registered on `builder.Services`; `[Route("api/[controller]")]` maps requests to `/api/Orders`, and `[HttpGet("{id}")]` combines with it to match `GET /api/Orders/{id}`' },
    ],
    answer: 'd',
  ...
  },
  {
    ...
    options: [
      { id: 'a', text: '`app.Run` registers **terminal** middleware — its delegate takes no `next` parameter, so it always produces a response and ends the pipeline right there. `Map*` calls do not insert middleware where they are written; they only add an entry to the endpoint data source. Because ASP.NET Core wires the implicit `UseRouting` in at the very start of the pipeline and the implicit `UseEndpoints` in at the very end, `/hello` is already matched by routing before this code even runs — but the terminal `app.Run`, registered before that implicit `UseEndpoints`, never calls `next`, so `UseEndpoints` is never reached and `fallback` wins. Fix: remove the catch-all `app.Run` (a real fallback should use `app.MapFallback(...)`, which only runs when no endpoint matched), or call `UseRouting()`/`UseEndpoints()` explicitly and place the terminal `app.Run` after `UseEndpoints()`' },
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'b', text: 'Minimal APIs require an explicit `[FromBody]` attribute on `request`; without it, the endpoint throws `InvalidOperationException` at startup because the binding source for a `record` parameter can never be inferred' },
      { id: 'c', text: '`orders` is inferred as a DI-supplied service because `IOrderService` is a registered service type, so it is resolved from the request\'s `IServiceProvider`; `request` matches no other binding source, so it falls back to being bound from the JSON request body. Only one parameter can be implicitly body-bound this way' },
      { id: 'd', text: '`orders` is bound from the body because it is declared second in the parameter list, and `request` is resolved from DI because `record` types are always treated as services by minimal API model binding' },
    ],
    answer: 'c',
