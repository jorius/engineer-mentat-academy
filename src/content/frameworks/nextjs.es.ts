...
export const translations: Record<string, QuestionTranslation> = {
  'nextjs-server-component-uses-state': {
    ...
    explanation:
      "En el App Router, todo componente es un **Server Component** por defecto: se ejecuta solo en el servidor y no envía JavaScript, así que no puede usar estado, efectos, event handlers ni APIs del navegador. `'use client'` marca una **frontera**: ese módulo y todo lo que importa pasan a formar parte del bundle del cliente. Coloca la frontera lo más abajo posible (lo más cerca de las hojas). Marcar el layout raíz empujaría todo el árbol al cliente y desperdiciaría la obtención de datos en el servidor y los beneficios en el tamaño del bundle. Ningún hook funciona en los Server Components, incluido `useRef`.",
  },
  'nextjs-route-handler-basics': {
    prompt: 'En el App Router, ¿cómo expones `GET /api/users` para que devuelva JSON?',
    options: {
      a: 'Crea `app/api/users/route.ts` que exporte `export default function handler(req, res) { res.json(users) }`',
  ...
  'nextjs-dynamic-api-opts-out-of-static': {
    ...
    explanation:
      "`cookies()`, `headers()`, `draftMode()`, `connection()` y la prop de página `searchParams` dependen de la petición entrante. Usar cualquiera de ellas (o `fetch` con `cache: 'no-store'`, o `export const dynamic = 'force-dynamic'`) hace que la ruta se renderice en cada petición. En Next.js 15 estas APIs son **async** (`await cookies()`; `params` y `searchParams` son promesas), y el acceso síncrono solo funciona mediante un shim de compatibilidad temporal que registra una advertencia. Para mantener estática la mayor parte de la página, lee la cookie dentro de un componente pequeño envuelto en `<Suspense>` (con Partial Prerendering esto se convierte en un hueco dinámico dentro de un shell estático), o convierte la moneda en el cliente. Los componentes `async` por sí solos se pueden prerenderizar sin problema, y `generateStaticParams` solo hace falta para segmentos dinámicos como `[id]`.",
  },
  ...
  'nextjs-caching-defaults-15': {
    ...
    options: {
      a: "Los resultados de `fetch` ya no se guardan en el Data Cache por defecto; actívalo por petición con `cache: 'force-cache'` o `next: { revalidate: N }`",
      b: 'Una ruta que no usa Dynamic APIs se sigue prerenderizando en tiempo de build, así que un `fetch` sin caché dentro de ella se ejecuta una vez durante `next build` y su resultado queda congelado en la salida estática hasta la revalidación o el siguiente deploy',
      c: "Llamar a `revalidateTag('products')` desde una Server Action o un Route Handler invalida todos los `fetch` en caché etiquetados con `products` (mediante `next: { tags: ['products'] }`), en todas las rutas que los usaron",
