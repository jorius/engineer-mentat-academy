...
export const translations: Record<string, QuestionTranslation> = {
  'express-middleware-registration-order': {
    ...
    hint:
      'Recuerda cómo decide Express qué capa de la pila se ejecuta a continuación para una petición.',
  },
  'express-error-handler-arity': {
    prompt:
      "```js\napp.get('/orders/:id', async (req, res) => {\n  throw new Error('db down');\n});\n\napp.use((err, req, res) => {\n  res.status(500).json({ error: err.message });\n});\n```\nEn **Express 5**, el manejador de errores personalizado nunca se ejecuta; los clientes reciben la página de error HTML por defecto de Express. ¿Qué está mal?",
    options: {
    ...
    hint:
      'Recuerda exactamente cómo reconoce Express un middleware de manejo de errores, y qué hace Express 5 con un handler async rechazado.',
  },
  'express-async-errors-v4-vs-v5': {
    ...
    options: {
      ...
      b: 'Express 4 nunca ve el rechazo: se convierte en un unhandled rejection que por defecto tumba el proceso en Node 15+ (la conexión del cliente se corta; si un listener de `unhandledRejection` mantiene vivo el proceso, la petición se queda colgada); Express 5 reenvía la promesa rechazada a `next(err)`, así que el manejador de errores responde con un 500',
  ...
  'express-next-semantics': {
    ...
    hint:
      'Recuerda qué hace cada forma de llamar a `next`, y qué termina realmente la ejecución de una función middleware.',
  },
  'express-route-order-param-shadowing': {
    ...
    hint:
      'Recuerda cómo compara Express la ruta de una petición con las rutas registradas en un router.',
  },
  'express-error-handler-headers-sent': {
    prompt:
      "```js\napp.get('/export.csv', async (req, res) => {\n  res.setHeader('Content-Type', 'text/csv');\n  for await (const row of db.streamRows()) {\n    res.write(toCsv(row)); // the DB connection drops half-way\n  }\n  res.end();\n});\n\napp.use((err, req, res, next) => {\n  logger.error(err);\n  res.status(500).json({ error: 'internal' });\n});\n```\nEn **Express 5**, ¿qué debería hacer el manejador de errores ante este fallo?",
    options: {
    ...
    hint:
      'Considera qué recibió ya el cliente cuando falla el stream, y qué se puede cambiar todavía de la respuesta en ese momento.',
  },
  'express-compose-middleware-fix': {
    ...
    hint:
      'Revisa qué le devuelve `next` al middleware que lo espera con `await`, y qué necesitaría recordar `compose` para notar una segunda llamada.',
  },
