...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'iac-terraform-drift-handling': {
    prompt:
      'Durante un incidente, alguien abrió el puerto 5432 en un security group administrado por Terraform desde la consola de AWS. Las reglas del grupo están declaradas como bloques `ingress` inline dentro de su recurso `aws_security_group`, no como recursos `aws_security_group_rule` o `aws_vpc_security_group_ingress_rule` separados. ¿Qué hace el siguiente `terraform plan` y cuál es la forma correcta de manejarlo?',
    options: {
      a: 'Nada: Terraform solo compara la configuración con el state file, que todavía tiene las reglas anteriores, así que la regla extra se queda hasta que alguien ejecute `terraform refresh`',
      b: 'El plan refresca el grupo, muestra la regla extra como drift y propone eliminarla; el equipo o bien codifica la regla en HCL o bien deja que el apply la elimine',
      c: 'Ejecutar `terraform import` sobre la nueva regla; eso basta para que Terraform la adopte, sin necesidad de cambiar la configuración',
      d: 'Terraform reescribe los archivos `.tf` para incluir el cambio de la consola en el siguiente plan, así que el pull request lo muestra para revisión',
    },
    explanation:
  ...
  'iac-cloudformation-stacks-change-sets': {
    ...
    options: {
      ...
      b: 'Una actualización fallida del stack hace rollback automáticamente por defecto; si el propio rollback falla, el stack queda en `UPDATE_ROLLBACK_FAILED` hasta que continúes el rollback, posiblemente omitiendo recursos',
