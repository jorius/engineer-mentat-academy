...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'serverless-cold-start-mitigation': {
    ...
    explanation:
      'Un cold start ocurre cada vez que Lambda tiene que crear un nuevo entorno de ejecución: la primera solicitud, un scale-out más allá de los entornos calientes actuales, un despliegue nuevo, o después de que se recupera un entorno inactivo. La **provisioned concurrency** mantiene N entornos inicializados (con costo). Los bundles más pequeños y un código de init liviano acortan cada cold start, y el trabajo de init hecho fuera del handler se reutiliza en las invocaciones en caliente. Más memoria también da proporcionalmente más CPU, lo que acelera el init. **SnapStart** toma un snapshot del entorno inicializado y lo restaura; revisa qué runtimes lo soportan (Java, Python y .NET al momento de escribir esto). Un ping mantiene caliente aproximadamente **un** entorno; una ráfaga de 50 solicitudes concurrentes igual necesita 49 nuevos. El timeout no tiene ningún efecto sobre el arranque.',
  },
  ...
  'serverless-cost-model-steady-load': {
    ...
    options: {
      a: 'Lambda siempre es lo más barato porque solo pagas cuando el código se ejecuta',
  ...
  'serverless-vendor-lock-in': {
    ...
    options: {
      a: 'En la firma del handler; envuelve cada función en un framework multi-nube para que los handlers nunca cambien',
      b: 'En la versión del runtime de Node.js que provee AWS; empaquetar tu propio runtime elimina el lock-in',
      c: 'En las fuentes de eventos, el modelo de IAM, los almacenes de datos administrados y los flujos de trabajo (DynamoDB, EventBridge, Step Functions) y las herramientas operativas; mantén la lógica de dominio en módulos simples detrás de ports and adapters, y acepta el acoplamiento a servicios administrados donde se paga solo',
      d: 'En ningún lugar importante: cualquier función Lambda se puede mover a otra nube volviendo a desplegar el zip',
    },
    explanation:
