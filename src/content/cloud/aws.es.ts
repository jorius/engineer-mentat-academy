...
export const translations: Record<string, QuestionTranslation> = {
  ...
  'aws-s3-strong-consistency': {
    ...
    explanation:
      'Desde diciembre de 2020, toda lectura en S3 después de una escritura exitosa devuelve los datos más recientes, para objetos nuevos, sobrescrituras, borrados y operaciones de listado, sin costo adicional. La respuesta a describe el modelo antiguo que muchos posts de blog todavía repiten. Consistencia fuerte no significa bloqueo: dos escritores concurrentes siguen produciendo last-writer-wins. Para concurrencia optimista usa **escrituras condicionales** (`If-None-Match: *` para crear solo si no existe, `If-Match` con un ETag para actualizar solo la versión que leíste).',
  },
  ...
  'aws-s3-event-parse-records': {
    prompt:
      'Una Lambda recibe notificaciones de eventos de S3. Implementa `solution(event)` para que devuelva `[{ bucket, key }]` por cada registro cuyo `eventName` empiece con `ObjectCreated:`, en el orden de los registros. Las keys de los objetos llegan **codificadas como URL**, con los espacios codificados como `+` (y a veces `%20`), así que decodifícalas para obtener la key real. Ignora otros tipos de evento y devuelve `[]` cuando falte `Records`.',
    explanation:
      'S3 codifica las keys de los objetos en las notificaciones como un formulario HTML: los espacios se convierten en `+` y todo lo demás se codifica con porcentaje, así que un `+` literal llega como `%2B`. El orden importa: reemplaza `+` por un espacio **primero** y luego aplica `decodeURIComponent`; si decodificas primero, `%2B` se convertiría en `+` y después, por error, en un espacio. Olvidar esto es un bug clásico de producción: `GetObject` con la key cruda devuelve `NoSuchKey` solo para los archivos con espacios o acentos. Además, filtra por `eventName` (o configura la notificación solo para `s3:ObjectCreated:*`), y recuerda que una invocación puede traer varios registros.',
  },
  ...
  'aws-s3-to-sns-required-wiring': {
    ...
    options: {
      ...
      c: 'Si el topic usa SSE-KMS, una customer managed key cuya key policy permita a `s3.amazonaws.com` llamar a `kms:GenerateDataKey*` y `kms:Decrypt`',
  ...
  'aws-cognito-user-pool-vs-identity-pool': {
    ...
    options: {
      a: 'Solo un user pool: S3 acepta su ID token como credencial',
      b: 'Un user pool para autenticar y emitir JWT, más un identity pool que intercambia el token por credenciales temporales de AWS de un rol IAM limitado con la variable de política `${cognito-identity.amazonaws.com:sub}`',
      c: 'Solo un identity pool: guarda los usuarios y las contraseñas y emite JWT',
      d: 'Un user pool más un Cognito authorizer de API Gateway, que le otorga a la app permisos sobre S3',
    },
    explanation:
      'Los **user pools** son el directorio de usuarios y el proveedor de identidad OIDC: registro, inicio de sesión, MFA, federación con proveedores sociales o SAML, y emiten tokens de ID, de acceso y de refresco (JWT) para *tus* APIs. Los **identity pools** (identidades federadas) no guardan usuarios; toman un token de un user pool o de otro proveedor y llaman a STS para entregar **credenciales temporales de AWS** de un rol IAM, de modo que el cliente pueda llamar directamente a los servicios de AWS. Las variables de política como `${cognito-identity.amazonaws.com:sub}` en la política del rol restringen cada identidad a su propio prefijo. Los servicios de AWS nunca aceptan directamente un JWT de user pool, y un authorizer de API Gateway solo protege las rutas de tu API. A menudo la alternativa más simple es omitir los identity pools y hacer que tu API devuelva presigned URLs.',
  },
};
